---
title: "GEOG0013: Geography in the Field 1 (GIF1) 2023/24"
site: bookdown::bookdown_site
output: bookdown::gitbook
documentclass: book
link-citations: yes
github-repo: "UCLPG-MSC-SGDS/GEOG0013"
description: "GEOG0013: Geography in the Field 1 (GIF1)"
bibliography: [book.bib, packages.bib]
editor_options: 
  markdown: 
    wrap: 72
---

# Welcome to Introduction to GIS {.unnumbered}

```{r echo=FALSE, out.width = "100%", fig.align='center', cache=TRUE,}
knitr::include_graphics('images/general/Composite_image.jpeg') 
```

Welcome to **GIF1's** **Introduction to GIS**. This resource focuses on the themes of **data collection**, **fieldwork** and **GIS** for story mapping and digital human geography. Here, we will provide you with the necessary information and training to prepare you for collecting the data needed to accomplish the tasks for the **Urban Project** and **Introduction to GIS**. 

## Structure {-}

This resource is split into **two** parts:

- **Part 1** of this resource contains the necessary self-guided training materials (i.e., guidance notes and demonstration videos) for using the smartphone applications such as **Google Earth APP** and **Strava** for collecting your data in London or Barcelona.
- **Part 2** contains the information needed to developing your own **Story Maps** using the online platform called **ArcGIS Online**. You will need the data collected from the activities in London or Barcelona to complete the exercise and assignment. The GIS workshops will be on **Wednesday 15th November**, and **Friday 17th November**. The learning content for **Part 2** will be released a week ahead of these dates.

**Contact information:**

Anwar Musah | Lecturer in Social & Geographic Data Science </br>
UCL Department of Geography </br>
University College London </br>
Room 115 North-West Wing, Gower Street, London, WC1E 6BT </br>
Email: a.musah@ucl.ac.uk
